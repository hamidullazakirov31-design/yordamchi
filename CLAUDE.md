...
- **Qaror:** Neon Postgres bazasiga ulanish uchun Neon serverless HTTP drayveri (443 port) ishlatildi, oddiy Postgres protokoli (5432 port) o'rniga; schema qo'llandi (`Task.goalId` ustuni + FK) va test ma'lumot (4 maqsad + 7 vazifa) seed qilindi. — **Sabab:** joriy muhitda 5432 porti bloklangan, HTTP orqali ulanish muqobil yechim sifatida ishladi. (Infratuzilma)
- **Qaror:** Vercel Production va Preview muhitlariga barcha zarur maxfiy o'zgaruvchilar (`TELEGRAM_BOT_TOKEN`, `AUTH_SECRET`, `TELEGRAM_WEBHOOK_SECRET`, `ALLOWED_TELEGRAM_ID`, `NEXT_PUBLIC_APP_URL`, `NEXT_PUBLIC_BOT_USERNAME`) qo'shildi va Telegram webhook (`/api/telegram/webhook`, secret_token bilan himoyalangan) o'rnatildi. — **Sabab:** bu kalitlar ilgari Vercel'da yo'q edi, shu sababli bot va login production'da ishlamayotgan edi; endi `/start` qabul xabari + login tugmasini qaytaradi. (Infratuzilma)
- **Qaror:** Kodga bir nechta ruxsat etilgan Telegram ID'ni qo'llab-quvvatlash qo'shildi (`env.allowedTelegramIds`, vergul bilan ajratilgan ro'yxat, `env.isAllowed()`) — commit `50a57f6`. — **Sabab:** kelajakda bir nechta foydalanuvchini ruxsat etish ehtiyoji. (Sprint 2)
- **Qaror:** Production hozircha eski kod versiyasida qoldirilmoqda (faqat env o'zgaruvchilar redeploy orqali qo'llandi); Sprint 2'ning yangi CRUD/Aurora dizayn kodi (branch'da tayyor) Vercel CLI source-deploy tiqilib qolgani sababli hali productionga chiqarilmagan va keyingi qadamda git orqali chiqariladi. Shu sababli `ALLOWED_TELEGRAM_ID` hozircha yakka qiymat (`1271615246`) bilan qoldirilgan, chunki eski kod vergulli ro'yxatni ajrata olmaydi. — **Sabab:** production'dagi ishlayotgan bot funksionalligini buzmaslik; yangi kod deploy qilingach ikkinchi ID qo'shiladi. (Infratuzilma)
